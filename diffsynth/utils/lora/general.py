...
class GeneralLoRALoader:
    ...
    def fuse_lora_to_base_model(self, model: torch.nn.Module, state_dict, alpha=1.0):
        ...
        lora_layer_names = set(
            [
                i.replace(".lora_B.weight", "")
                for i in state_dict
                if i.endswith(".lora_B.weight")
            ]
        )
        for name, module in model.named_modules():
            if name in lora_layer_names:
                weight_up = state_dict[name + ".lora_B.weight"].to(
                    device=self.device, dtype=self.torch_dtype
                )
                weight_down = state_dict[name + ".lora_A.weight"].to(
                    device=self.device, dtype=self.torch_dtype
                )
                ...
                updated_num += 1
        print(
            f"{updated_num} tensors are fused by LoRA. Fused LoRA layers cannot be cleared by `pipe.clear_lora()`."
        )
