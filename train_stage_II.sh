...
SPVC_STAGE_I_LOW_CHECKPOINT="${SPVC_STAGE_I_LOW_CHECKPOINT:-${SPVC_OUTPUT_ROOT}/Wan2.2-Fun-A14B-Control_low_noise_stage_I/step-2080.safetensors}"
SPVC_NUM_FRAMES="${SPVC_NUM_FRAMES:-25}"
ACCELERATE_CONFIG_FILE="${ACCELERATE_CONFIG_FILE:-${SPVC_ROOT}/accelerate_config.yaml}"

for SPVC_CHECKPOINT in "${SPVC_STAGE_I_HIGH_CHECKPOINT}" "${SPVC_STAGE_I_LOW_CHECKPOINT}"; do
  if [[ ! -f "${SPVC_CHECKPOINT}" ]]; then
    echo "Missing Stage I checkpoint: ${SPVC_CHECKPOINT}" >&2
    exit 1
  fi
done
...
SPVC_COMMON_ARGS=(
  --dataset_base_path "${SPVC_STAGE_II_DATA_ROOT}"
  --dataset_metadata_path "${SPVC_STAGE_II_METADATA}"
  --data_file_keys "video,control_video,reference_video,cam_pose,reference_combined_video"
  --height 448
  --width 800
...
"${SPVC_ACCELERATE[@]}" examples/wanvideo/model_training/train.py \
  ...
  --output_path "${SPVC_OUTPUT_ROOT}/Wan2.2-Fun-A14B-Control_high_noise_stage_II" \
...
"${SPVC_ACCELERATE[@]}" examples/wanvideo/model_training/train.py \
  ...
  --output_path "${SPVC_OUTPUT_ROOT}/Wan2.2-Fun-A14B-Control_low_noise_stage_II" \
