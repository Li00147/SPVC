...
from ..configs.vram_management_module_maps import VRAM_MANAGEMENT_MODULE_MAPS
from ..core.loader.file import hash_model_file
from ..core.loader.model import load_model
from ..core.vram.layers import AutoWrappedModule
...
class ModelPool:
    ...
    def import_model_class(self, model_class):
        ...
        model_class = importlib.import_module(model_resource).__getattribute__(
            model_class
        )
        return model_class
    ...
    def need_to_enable_vram_management(self, vram_config):
        return (
            vram_config["offload_dtype"] is not None
        ...
        if self.need_to_enable_vram_management(vram_config):
            if model_class in VRAM_MANAGEMENT_MODULE_MAPS:
                module_map = {
                    ...
                    for source, target in VRAM_MANAGEMENT_MODULE_MAPS[
                        model_class
                    ].items()
                }
            else:
    ...
    def load_model_file(self, config, path, vram_config, vram_limit=None):
        ...
        if "state_dict_converter" in config:
            state_dict_converter = self.import_model_class(
                config["state_dict_converter"]
            )
        else:
    ...
    def auto_load_model(
        ...
        if not loaded:
            raise ValueError(
                f"Cannot detect the model type. File: {path}. Model hash: {model_hash}"
            )
    ...
    def fetch_model(self, model_name, index=None):
        ...
        if len(fetched_models) == 0:
            ...
            print(
                f"Using {model_name} from {json.dumps(fetched_model_paths[0], indent=4)}."
            )
