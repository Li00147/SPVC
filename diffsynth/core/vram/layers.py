...
from ..device.npu_compatible_device import parse_device_type
...
class AutoTorchModule(torch.nn.Module):
    ...
    def check_free_vram(self):
        device = (
            self.computation_device if self.computation_device != "npu" else "npu:0"
        )
        gpu_mem_state = getattr(torch, self.computation_device_type).mem_get_info(
            device
        )
...
class AutoWrappedModule(AutoTorchModule):
    ...
    def onload(self):
        if self.state < 1:
            if (
                self.disk_offload
                and self.onload_device != "disk"
                and (self.offload_device == "disk")
            ):
    ...
    def preparing(self):
        if self.state != 2:
            if (
                self.disk_offload
                and self.preparing_device != "disk"
                and (self.onload_device == "disk")
            ):
    ...
    def computation(self):
        ...
        if torch_dtype == self.computation_dtype and device == self.computation_device:
            ...
            module = self.load_from_disk(
                self.computation_dtype, self.computation_device, copy_module=True
            )
        else:
            module = self.cast_to(
                self.module,
...
class AutoWrappedLinear(torch.nn.Linear, AutoTorchModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.enable_fp8 = computation_dtype in [
            torch.float8_e4m3fn,
    ...
    def load_from_disk(self, torch_dtype, device, assign=True):
        weight = self.disk_map[self.name + ".weight"].to(
            dtype=torch_dtype, device=device
        )
        bias = (
            None
            if self.bias is None
            else self.disk_map[self.name + ".bias"].to(dtype=torch_dtype, device=device)
        )
        if assign:
    ...
    def computation(self):
        ...
        if torch_dtype == self.computation_dtype and device == self.computation_device:
            ...
            weight, bias = self.load_from_disk(
                self.computation_dtype, self.computation_device, assign=False
            )
        else:
            weight = self.cast_to(
    ...
    **kwargs,
    ...
    for source_module, target_module in module_map.items():
        if isinstance(model, source_module):
            ...
            model = target_module(
                model, **vram_config, vram_limit=vram_limit, disk_map=disk_map, **kwargs
            )
            break
    else:
        enable_vram_management_recursively(
